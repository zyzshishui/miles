#!/bin/bash
# Qwen3-30B-A3B: P2P RDMA weight transfer, 1 train + 1 rollout
#
# Usage (2 nodes):
#   Node 1 (head/train):  bash scripts/run-qwen3-30B-A3B-p2p.sh head
#   Node 2 (rollout):     MASTER_ADDR=<head_ip> bash scripts/run-qwen3-30B-A3B-p2p.sh worker
#   Node 1 (submit):      nohup bash scripts/run-qwen3-30B-A3B-p2p.sh submit > nohup_p2p_rdma.out
...
run_head() {
    ...
    echo "=== Ray Head (30B P2P) IP=${MASTER_ADDR} GPUs=${NUM_GPUS} ==="
    ray start --head \
...
run_worker() {
    ...
    echo "=== Ray Worker (30B P2P) Head=${MASTER_ADDR} Worker=${worker_ip} ==="
    ray start \
