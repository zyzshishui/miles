#!/bin/bash
# Usage:
#   bash scripts/run-qwen3-4B-amd-2node-colocate.sh head
#   MASTER_ADDR=${MASTER_ADDR} bash scripts/run-qwen3-4B-amd-2node-colocate.sh worker
#   nohup bash scripts/run-qwen3-4B-amd-2node-colocate.sh submit > nohup_colocated.out
...
run_head() {
    ...
    echo "=============================================="
    echo "Starting Ray Head Node (Colocated Mode)"
    ...
    echo ""
    echo "Head started. Run on worker node:"
...
run_worker() {
    ...
    echo "=============================================="
    echo "Starting Ray Worker Node (Colocated Mode)"
    echo "Head IP: ${MASTER_ADDR}"
...
run_submit() {
    ...
    WANDB_ARGS=(
        ...
        --wandb-group qwen3-4B-2node-colocate
    )
    ...
    echo "=============================================="
    echo "Submitting Colocated Training Job"
    echo "Mode: 2 nodes, train + rollout colocated"
    echo "=============================================="
...
show_usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  head    - Start Ray head node (run on head machine)"
    echo "  worker  - Start Ray worker node (run on worker machine, needs MASTER_ADDR)"
    echo "  submit  - Submit training job (run on head machine after both nodes started)"
    echo ""
    echo "Example:"
    echo "  # On head node:"
    echo "  bash $0 head"
    echo ""
    echo "  # On worker node:"
    echo "  MASTER_ADDR=10.28.104.180 bash $0 worker"
    echo ""
    echo "  # On head node (submit job):"
    echo "  bash $0 submit"
}
