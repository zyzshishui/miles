#!/bin/bash
# Usage (3 nodes: 1 train + 2 rollout, disaggregate async):
#   Node 1 (head/train):    bash scripts/run-qwen3-235B-A22B-Instruct-2507-amd.sh head
#   Node 2-3 (rollout):     MASTER_ADDR=<head_ip> bash scripts/run-qwen3-235B-A22B-Instruct-2507-amd.sh worker
...
")
    ...
    export NCCL_IB_HCA=${NCCL_IB_HCA:-rocep9s0,rocep25s0,rocep105s0,rocep121s0,rocep137s0,rocep153s0,rocep233s0,rocep249s0}
    if [[ -z "${NCCL_IB_GID_INDEX+x}" ]]; then
        local first_hca=$(echo "$NCCL_IB_HCA" | cut -d',' -f1)
        if [[ -r /sys/class/infiniband/${first_hca}/ports/1/gids/1 ]] && [[ "$(cat /sys/class/infiniband/${first_hca}/ports/1/gids/1)" != "0000:0000:0000:0000:0000:0000:0000:0000" ]]; then
...
run_submit() {
    ...
    # 235B-A22B: TP=4, PP=1, EP=8 (1 train node x 8 GPUs, DP=2)
    # TP max=4 (limited by num_query_groups=4), EP=8: MoE all-to-all intra-node
    PERF_ARGS=(
    ...
    # 2 rollout nodes x 8 GPUs = 16, each engine = 1 node (8 GPUs)
    SGLANG_ARGS=(
    ...
    echo "Mode: 1 node train + 2 nodes rollout (async)"
    echo "Train: 1 x 8 GPUs (TP=4, PP=1, EP=8, DP=2)"
    echo "Rollout: 2 x 8 GPUs = 16"
