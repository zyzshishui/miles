#!/bin/bash
# Usage:
#   bash scripts/run-qwen3-4B-amd-2node-disagg.sh head   
#   MASTER_ADDR=${MASTER_ADDR} bash scripts/run-qwen3-4B-amd-2node-disagg.sh worker
#   nohup bash scripts/run-qwen3-4B-amd-2node-disagg.sh submit > nohup_disaggregated.out
...
run_submit() {
    ...
    WANDB_ARGS=(
        ...
        --wandb-group qwen3-4B-2node-disagg
        --wandb-key ${WANDB_KEY} 
    )
    ...
    echo "Mode: 1 node train + 1 node rollout"
    echo "=============================================="
