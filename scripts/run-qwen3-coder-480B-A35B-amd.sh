#!/bin/bash
# Usage (4 nodes: 3 train + 1 rollout):
#   Node 1 (head/train):   bash scripts/run-qwen3-coder-480B-A35B-amd.sh head
#   Node 2-3 (train):      MASTER_ADDR=<head_ip> bash scripts/run-qwen3-coder-480B-A35B-amd.sh worker
#   Node 4 (rollout):      MASTER_ADDR=<head_ip> bash scripts/run-qwen3-coder-480B-A35B-amd.sh worker
#   Node 1 (submit):       nohup bash scripts/run-qwen3-coder-480B-A35B-amd.sh submit > nohup_disaggregated.out
#
...
run_submit() {
    ...
    WANDB_ARGS=(
        ...
        --wandb-group qwen3-coder-480B-6node-disagg
        --wandb-key ${WANDB_KEY}
    )
    ...
    echo "Mode: 4 nodes train + 2 nodes rollout"
    echo "=============================================="
