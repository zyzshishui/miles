#!/bin/bash
# Usage:
#   bash scripts/launch-multi-node.sh start    # Start all nodes
#   bash scripts/launch-multi-node.sh stop     # Stop all nodes
#   bash scripts/launch-multi-node.sh status   # Check status
#   bash scripts/launch-multi-node.sh submit   # Submit training job

set -euo pipefail

# ============== Configuration ==============
# Node list (first one is head node)
NODES=(
    "mi355-gpu-52"    # Head node
    "mi355-gpu-53"
    "mi355-gpu-54"
)

# Docker configuration
DOCKER_IMAGE="rlsys/miles:MI350-355-latest"
CONTAINER_NAME="yuzhen_miles_0318"
SHARED_ROOT="/shared/data/yuzhenzhou"
WORKSPACE_DIR="/workspace"
MILES_DIR="${WORKSPACE_DIR}/miles"

# Training script
TRAIN_SCRIPT="scripts/run-qwen3-235B-A22B-Instruct-2507-amd.sh"
# TRAIN_SCRIPT="scripts/run-qwen3-4B-amd.sh"

# SSH options
SSH_OPTS="-o StrictHostKeyChecking=no -o ConnectTimeout=30"

# ============== Helper Functions ==============
LOCAL_HOSTNAME="$(hostname -s)"

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"
}

is_local() {
    [[ "$1" == "${LOCAL_HOSTNAME}" || "$1" == "$(hostname)" ]]
}

run_on() {
    local host=$1
    shift
    if is_local "$host"; then
        bash -c "$*"
    else
        ssh ${SSH_OPTS} "$host" "$@"
    fi
}

docker_exec() {
    local host=$1
    shift
    run_on "$host" "docker exec ${CONTAINER_NAME} bash -c '$*'"
}

check_container() {
    local host=$1
    run_on "$host" "docker ps -q -f name=^${CONTAINER_NAME}\$" 2>/dev/null | grep -q .
}

get_container_ip() {
    local host=$1
    run_on "$host" "docker exec ${CONTAINER_NAME} hostname -I" | awk '{print $1}'
}

# ============== Start Docker Containers ==============
start_containers() {
    log "Starting Docker containers on all nodes..."

    for host in "${NODES[@]}"; do
        log "Checking $host..."

        if check_container "$host"; then
            log "  Container already running on $host"
        else
            log "  Starting container on $host..."
            run_on "$host" "docker run -itd --network=host --privileged --device=/dev/kfd --device=/dev/dri \
                --device=/dev/infiniband \
                --ipc=host --shm-size 16G --group-add video --cap-add=SYS_PTRACE \
                --security-opt seccomp=unconfined \
                --ulimit nofile=1048576:1048576 \
                -v ${SHARED_ROOT}:${WORKSPACE_DIR} \
                -w ${MILES_DIR} \
                -e PIP_CACHE_DIR=${WORKSPACE_DIR}/.cache/pip \
                -e TORCH_HOME=${WORKSPACE_DIR}/.cache/torch \
                -v /usr/lib/x86_64-linux-gnu/libionic.so:/usr/lib/x86_64-linux-gnu/libionic.so:ro \
                -v /usr/lib/x86_64-linux-gnu/libionic.so.1:/usr/lib/x86_64-linux-gnu/libionic.so.1:ro \
                -v /usr/lib/x86_64-linux-gnu/libionic.so.1.0.54.0-119.gbd4ea0970:/usr/lib/x86_64-linux-gnu/libionic.so.1.0.54.0-119.gbd4ea0970:ro \
                -v /usr/lib/x86_64-linux-gnu/libibverbs/libionic-rdmav34.so:/usr/lib/x86_64-linux-gnu/libibverbs/libionic-rdmav34.so:ro \
                -v /etc/libibverbs.d/ionic.driver:/etc/libibverbs.d/ionic.driver:ro \
                -v ${SHARED_ROOT}/amd-anp/build:/opt/rocm/lib/rccl-net-plugin:ro \
                -v /boot/config-\$(uname -r):/boot/config-\$(uname -r):ro \
                -e LD_LIBRARY_PATH=/opt/rocm/lib/rccl-net-plugin:/opt/rocm/lib \
                -e NCCL_NET_PLUGIN=anp \
                -e HSA_FORCE_FINE_GRAIN_PCIE=1 \
                -e HSA_NO_SCRATCH_RECLAIM=1 \
                -e NCCL_NET_GDR_LEVEL=3 \
                -e NCCL_NET_OPTIONAL_RECV_COMPLETION=0 \
                -e NCCL_GDR_FLUSH_DISABLE=1 \
                -e RCCL_GDR_FLUSH_GPU_MEM_NO_RELAXED_ORDERING=0 \
                -e NCCL_IB_USE_INLINE=1 \
                -e IONIC_LOCKFREE=all \
                -e NCCL_MIN_NCHANNELS=64 \
                -e NCCL_MAX_NCHANNELS=64 \
                -e NCCL_NCHANNELS_PER_NET_PEER=16 \
                -e NCCL_IB_QPS_PER_CONNECTION=1 \
                -e NCCL_IB_TC=96 \
                -e NCCL_IB_FIFO_TC=184 \
                -e NCCL_IB_GID_INDEX=1 \
                -e NCCL_IB_PCI_RELAXED_ORDERING=1 \
                -e NCCL_PXN_DISABLE=1 \
                -e NCCL_IGNORE_CPU_AFFINITY=1 \
                -e NCCL_P2P_NET_CHUNKSIZE=262144 \
                -e RCCL_P2P_SHIFT_SIZE=0 \
                -e NCCL_GDRCOPY_ENABLE=0 \
                -e NCCL_DMABUF_ENABLE=0 \
                -e HF_HOME=${WORKSPACE_DIR}/.cache/huggingface \
                -e TRANSFORMERS_CACHE=${WORKSPACE_DIR}/.cache/huggingface \
                -e HF_DATASETS_CACHE=${WORKSPACE_DIR}/.cache/huggingface/datasets \
                -e WANDB_KEY='cd411df8b73eb3f5c1ae1220cc1ec4e3c9d1f86e' \
                -e WANDB_API_KEY='cd411df8b73eb3f5c1ae1220cc1ec4e3c9d1f86e' \
                -e DEPRECATED_MEGATRON_COMPATIBLE=1 \
                --name ${CONTAINER_NAME} \
                ${DOCKER_IMAGE}" || log "  Warning: Container might already exist on $host"
        fi
    done

    log "All containers started."
}

# ============== Install Miles ==============
install_miles() {
    local tms_dir="${WORKSPACE_DIR}/torch_memory_saver"
    local head_node="${NODES[0]}"

    local tms_host_dir="${SHARED_ROOT}/torch_memory_saver"

    log "Installing torch_memory_saver..."
    docker_exec "${head_node}" "rm -rf ${tms_dir}/torch_memory_saver.egg-info ${tms_dir}/build" 2>/dev/null || true
    rm -rf "${tms_host_dir}" 2>/dev/null || true
    git clone https://github.com/zyzshishui/torch_memory_saver.git "${tms_host_dir}"
    chmod -R a+w "${tms_host_dir}"
    for host in "${NODES[@]}"; do
        docker_exec "$host" "cd ${tms_dir} && pip install -e . -q" &
    done
    wait
    log "torch_memory_saver installed on all nodes."

    log "Installing miles on all nodes..."
    chmod -R o+w "${SHARED_ROOT}/miles" 2>/dev/null || true
    for host in "${NODES[@]}"; do
        log "Installing on $host..."
        docker_exec "$host" "cd ${MILES_DIR} && pip install -e . -q" &
    done
    wait
    log "Miles installed on all nodes."
}

# ============== Start Ray Cluster ==============
start_ray_cluster() {
    log "Starting Ray cluster..."

    local head_node="${NODES[0]}"
    local head_ip

    head_ip=$(get_container_ip "$head_node")
    log "Head node: $head_node (IP: $head_ip)"

    log "Starting Ray head on $head_node..."
    docker_exec "$head_node" "cd ${MILES_DIR} && bash ${TRAIN_SCRIPT} head"

    log "Verifying Ray head is running..."
    local retries=0
    while ! docker_exec "$head_node" "ray status" &>/dev/null; do
        retries=$((retries + 1))
        if [ $retries -ge 6 ]; then
            log "ERROR: Ray head failed to start on $head_node after 30s. Aborting."
            exit 1
        fi
        log "  Waiting for head node... (${retries}/6)"
        sleep 5
    done
    log "Ray head is running."

    for ((i=1; i<${#NODES[@]}; i++)); do
        local worker_node="${NODES[$i]}"
        log "Starting Ray worker on $worker_node..."
        docker_exec "$worker_node" "cd ${MILES_DIR} && MASTER_ADDR=${head_ip} bash ${TRAIN_SCRIPT} worker" &
    done

    log "Waiting for workers to join..."
    sleep 15

    log "Checking Ray cluster status..."
    docker_exec "$head_node" "ray status" || true

    log "Ray cluster started. Head IP: $head_ip"
    echo ""
    echo "=============================================="
    echo "Ray cluster is ready!"
    echo "Head node: $head_node ($head_ip)"
    echo "Worker nodes: ${NODES[@]:1}"
    echo ""
    echo "To submit training job, run:"
    echo "  bash scripts/launch-multi-node.sh submit"
    echo "=============================================="
}

# ============== Submit Job ==============
submit_job() {
    local head_node="${NODES[0]}"

    log "Submitting training job from $head_node..."
    docker_exec "$head_node" "cd ${MILES_DIR} && nohup bash ${TRAIN_SCRIPT} submit > nohup_train.out 2>&1 &"

    log "Job submitted. Check logs with:"
    echo "  ssh $head_node 'docker exec ${CONTAINER_NAME} tail -f ${MILES_DIR}/nohup_train.out'"
}

# ============== Stop Cluster ==============
stop_cluster() {
    log "Stopping Ray cluster on all nodes..."

    for host in "${NODES[@]}"; do
        log "Stopping Ray on $host..."
        docker_exec "$host" "pkill -9 sglang; ray stop --force; pkill -9 ray; pkill -9 python; rm -rf /tmp/ray/" 2>/dev/null || true
    done

    log "Ray cluster stopped."
}

# ============== Remove Containers ==============
remove_containers() {
    for host in "${NODES[@]}"; do
        log "Removing container on $host..."
        run_on "$host" "docker rm -f ${CONTAINER_NAME}" 2>/dev/null || true
    done

    log "All containers removed."
}

# ============== Status ==============
show_status() {
    log "Checking status of all nodes..."
    echo ""

    for host in "${NODES[@]}"; do
        echo "=== $host ==="
        if check_container "$host"; then
            echo "  Container: Running"
            local ip=$(get_container_ip "$host" 2>/dev/null || echo "unknown")
            echo "  IP: $ip"
            echo "  Ray status:"
            docker_exec "$host" "ray status 2>/dev/null | head -10" || echo "    Ray not running"
        else
            echo "  Container: Not running"
        fi
        echo ""
    done
}

# ============== Full Start ==============
full_start() {
    log "Starting full multi-node setup..."

    start_containers
    sleep 5

    install_miles
    sleep 3

    start_ray_cluster
}

# ============== Main ==============
show_usage() {
    echo "Usage: $0 {start|stop|status|submit|containers|install|ray|stop-ray}"
    echo ""
    echo "Commands:"
    echo "  start       - Full start: containers + install + ray cluster"
    echo "  stop        - Stop everything: ray + containers"
    echo "  status      - Show status of all nodes"
    echo "  submit      - Submit training job"
    echo ""
    echo "Advanced commands:"
    echo "  containers  - Start Docker containers only"
    echo "  install     - Install miles only"
    echo "  ray         - Start Ray cluster only"
    echo "  stop-ray    - Stop Ray cluster only (keep containers)"
}

if [ $# -lt 1 ]; then
    show_usage
    exit 1
fi

case "$1" in
    start)
        full_start
        ;;
    stop)
        stop_cluster
        remove_containers
        ;;
    status)
        show_status
        ;;
    submit)
        submit_job
        ;;
    containers)
        start_containers
        ;;
    install)
        install_miles
        ;;
    ray)
        start_ray_cluster
        ;;
    stop-ray)
        stop_cluster
        ;;
    *)
        echo "Error: Unknown command '$1'"
        show_usage
        exit 1
        ;;
esac
