...
async def generate_with_miles_router(args, sample: Sample, sampling_params) -> Sample:
    """Generate using MilesRouter with text-based workflow"""
    ...
    # Prepare payload for MilesRouter (text-based)
    payload = {
        "text": full_text,
    ...
    # Call MilesRouter /generate endpoint
    ...
    # Get token IDs and logprobs using MilesRouter's /retrieve_from_text
    ...
    if "logp" in retrieve_output:
        # For MilesRouter, we get the full logprobs - need to extract only response ones
